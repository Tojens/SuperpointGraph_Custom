...
def loader(entry, train, args, db_path, test_seed_offset=0):
    ...
    if len(G.get_edgelist()) != 0:
        ...
        #print("length of clouds", len(clouds))
        if len(clouds) != 0:
...
def eccpc_collate(batch):
    ...
    if len(clouds_meta[0]) > 0:
        ...
        clouds_global = torch.cat([torch.from_numpy(np.asarray(f)) for f in clouds_global if f is not None], 0)
        clouds_flag = torch.cat([torch.from_numpy(np.asarray(f)) for f in clouds_flag if f is not None], 0)
        clouds = torch.cat([torch.from_numpy(np.asarray(f)) for f in clouds if f is not None], 0)
